...
public class XMLReader {
	...
	public void parserXMI() {
		try {
			// Utilizacao do DOM
			File file = new File(
					"src/parser/arquivo/profe.uml2");
			...
			for (int s = 0; s < nodeLst.getLength(); s++) {
				...
				// Para cada associacao, cria uma classe Association e poe num
				// vector.
				if (fstNode.getAttribute("xmi:type").equalsIgnoreCase(
						"uml:Association")) {
					String id = fstNode.getAttribute("xmi:id");
					String members = fstNode.getAttribute("memberEnd");
					String member1 = members.substring(0, 24);
					String member2 = members.substring(24, 47);
					String idChild = "";
					String typeChild = "";
					String assChild = "";

					NodeList children = fstNode
							.getElementsByTagName("ownedEnd");
					if (children != null) {
						Element child = (Element) children.item(0);
						if (child != null) {
							idChild = child.getAttribute("xmi:id");
							typeChild = child.getAttribute("type");
							assChild = child.getAttribute("association");
						}
					}
					Association ass = new Association(id, member1, member2,
							idChild, typeChild, assChild);
					myAssociationVector.add(ass);
				}

				else if (!fstNode.getAttribute("xmi:type").equalsIgnoreCase(
						"uml:Package")) {
					...
					if (type.equalsIgnoreCase("uml:Class")) {
						...
						if (children2 != null) {
							for (int i = 0; i < children2.getLength(); i++) {
								...
								if (child2 != null) {
									String operId = child2
											.getAttribute("xmi:id");
									String operName = child2
											.getAttribute("name");
									String operType = child2
											.getAttribute("type");

									// Faz uma lista das tags interna de uma tag
									// interna (ownedOperation e internamente
									// ownedParameter)
									NodeList childFromChild2_first = child2
											.getElementsByTagName("ownedParameter");
									// Faz uma lista das tags interna e uma tag
									// interna (ownedOperation e internamente
									// returnResult)
									NodeList childFromChild2_second = child2
											.getElementsByTagName("returnResult");

									if (childFromChild2_first != null) {
										for (int j = 0; j < childFromChild2_first
												.getLength(); j++) {

											// Pega cada tag interna do tipo
											// ownedOperation, ownedParameter
											Element childFromChild2_1 = (Element) childFromChild2_first
													.item(j);

											String idOperAtt = childFromChild2_1
													.getAttribute("xmi:id");
											String idOperName = childFromChild2_1
													.getAttribute("name");
											String idOperType = childFromChild2_1
													.getAttribute("type");
											String idOperDirection = childFromChild2_1
										...
										for (int j = 0; j < childFromChild2_second
												.getLength(); j++) {

											// Pega cada tag interna do tipo
											// ownedOperation, returnResult
											Element childFromChild2_1 = (Element) childFromChild2_second
													.item(j);

											String idResultID = childFromChild2_1
													.getAttribute("xmi:id");
											String idResultType = childFromChild2_1
													.getAttribute("type");
											String idResultDirection = childFromChild2_1
													.getAttribute("direction");
						...
						NodeList children = fstNode
								.getElementsByTagName("ownedLiteral");
